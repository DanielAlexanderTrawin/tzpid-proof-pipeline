...
def build_rows() -> list[dict[str, str | int]]:
    ...
    core_rows = [
        ...
        dict(
            ...
            level="focus_certificate_plus_phase4_phase5_phase5_5_spatial_boundary_plus_phase6_circulation_diagnostic",
            typed_carrier="yes",
            assumptions="yes",
            isabelle="clean_build",
            wolfram="yes",
            priority="critical",
            completion="phase5_6_spatial_boundary_locked_phase6_circulation_candidate_failed",
            next_upgrade="Next recompute Phase 6 circulation around a loop enclosing a wrapped phase singularity or vortex core; current HDF5 does not satisfy 2*pi*m quantization.",
        ),
        dict(
...
def write_md(rows: list[dict[str, str | int]]) -> None:
    ...
    upgrade_order = [
        ...
        (
            ...
            "Central movement-mechanism spine with Wolfram-backed checks plus Phase 4, Phase 5, Phase 5.5, Phase 5.6, and a Phase 6 circulation diagnostic.",
            "Recompute the Phase 6 circulation check around a wrapped phase/vortex-core loop so the candidate can attempt a real 2*pi*m quantization lock.",
        ),
        (
