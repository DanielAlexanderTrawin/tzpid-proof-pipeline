...
def build_rows() -> list[dict[str, str | int]]:
    ...
    core_rows = [
        ...
        dict(
            ...
            queue_status="",
            level="energy_matter_focus_plus_threshold_density_mass_energy_curvature_carrier_plus_temporal_emergence_flow_plus_criticality_split",
            typed_carrier="yes",
            assumptions="yes",
            isabelle="clean_build",
            wolfram="python_certificate",
            priority="critical",
            completion="matter_creation_criticality_bridge_locked",
            next_upgrade="Next introduce a concrete p(rho_vac) equation-of-state candidate and prove/compute its stationary points.",
        ),
        dict(
...
def write_md(rows: list[dict[str, str | int]]) -> None:
    ...
    upgrade_order = [
        ...
        (
            ...
            "Locks the Energy-to-Matter Logic spine as a real-valued pressure-threshold, density-gain, mass-energy, curvature-source, temporal-transfer, and ID10117 criticality-split carrier with Wolfram-backed checks plus a Python certificate for rho_vac(t), J_emerge(t), rho_matter(t), and total-transfer conservation.",
            "Introduce a concrete p(rho_vac) equation-of-state candidate and prove/compute its stationary points.",
        ),
        (
