...
def build_rows() -> list[dict[str, str | int]]:
    ...
    core_rows = [
        ...
        dict(
            ...
            queue_status="",
            level="hubble_breathing_closed_distance_plus_friedmann_pantheon_raw_likelihood",
            typed_carrier="yes",
            assumptions="yes",
            isabelle="clean_build",
            wolfram="python_certificate",
            priority="critical",
            completion="hubble_breathing_pantheon_raw_likelihood_locked",
            next_upgrade="Next add raw Planck/CMB distance-prior and DESI DR2 BAO covariance likelihood lanes, then combine all chi-square terms.",
        ),
        dict(
...
def write_md(rows: list[dict[str, str | int]]) -> None:
    ...
    upgrade_order = [
        ...
        (
            ...
            "Supplies the clock, observable distance fingerprint, typed Friedmann carrier, CPL certificate, observed-summary residual lane, and Pantheon+ raw covariance likelihood.",
            "Add raw Planck/CMB distance-prior and DESI DR2 BAO covariance likelihood lanes, then combine all chi-square terms.",
        ),
        (
