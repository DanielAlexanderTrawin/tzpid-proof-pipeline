...
def build_rows() -> list[dict[str, str | int]]:
    ...
    core_rows = [
        dict(
            ...
            level="hol_analysis_integral_proved",
            typed_carrier="yes",
            assumptions="yes",
            isabelle="clean_build",
            wolfram="yes",
            priority="critical",
            completion="phase2_5_hol_integral_set",
            next_upgrade="Next decide whether to promote Bessel special-function zeros from Wolfram certificates into a selected HOL special-function library or keep them as certified external numerics.",
        ),
        dict(
...
def write_md(rows: list[dict[str, str | int]]) -> None:
    ...
    upgrade_order = [
        (
            ...
            "The causal exponential kernel now has a HOL-Analysis improper integral proof: integral over {0..} equals 1 for positive decay time.",
        ),
        (
